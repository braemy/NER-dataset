#!/usr/bin/env bash

#remove the output in hdfs in case it already exists
hadoop fs -rm -r hdfs:///user/braemy/wikipedia_cleaned_$1$2.parquet
#run the cleaning
if [ "$#" -eq 2 ];
then
    spark-submit --master yarn --num-executors 150 --executor-memory 2G --conf spark.yarn.executor.memoryOverhead=1024 clean_wikipedia.py --id_max $1 --subpart $2
else
    spark-submit --master yarn --num-executors 150 --executor-memory 2G --conf spark.yarn.executor.memoryOverhead=1024 clean_wikipedia.py --id_max $1
fi

# Alternative titles
rm -rf /dlabdata1/braemy/wikidataNER/alternative_titles2/
#get the data from hdfs to file sysmte
hadoop fs -get hdfs:///user/braemy/alternative_titles2.json/ /dlabdata1/braemy/wikidataNER/alternative_titles2

python3 convert_to_dict.py